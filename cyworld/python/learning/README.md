...
> 그대로 적용해, 4개 MVP 도메인(일촌·미니홈피·방명록·도토리)을 계획·구현했습니다.
...
- `server/contexts/{chon,minihomepy,guestbook,acorn,today}/service.py` : 도메인 구현
- `server/contexts/minihomepy/screens.py` : 화면 렌더(순수 함수)
- `server/web/app.py` : WSGI 웹서버(라우팅 + 시드 데이터)
- `tests/test_{chon,minihomepy,guestbook,acorn,today}.py`,
  `tests/test_minihomepy_screen.py`, `tests/test_web_app.py` : proof 게이트 테스트
...
- 방명록·투데이는 사용자 존재 여부 외 다른 도메인에 직접 의존하지 않는다.

## 검증
```bash
python3 -m pip install -r requirements.txt
python3 proof/run_proof.py        # → 51/51 PASS (exit 0)
```

## 남은 범위 (다음 세션)
- 계획된 5개 도메인 + 미니홈피 화면·웹서버는 전부 구현·검증 완료.
- 실 로그인/세션 없음(데모는 `viewer` 쿼리 파라미터로 신원 대체).
- `sdd/05_operate` : 배포·모니터링 기록 (아직 롤아웃 없음).
